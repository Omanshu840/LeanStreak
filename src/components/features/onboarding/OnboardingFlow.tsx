...
export function OnboardingFlow({ onComplete, submitting = false }: Props) {
  ...
  return (
    ...
    >
      <div className="border-b border-[#e1e8f7] bg-[linear-gradient(135deg,#edffef_0%,#f4f9ff_46%,#fff8e8_100%)] px-5 py-4">
        <p className="text-xs font-bold uppercase tracking-wide text-[#5f6e8d]">Step {Math.min(step, 6)} of 6</p>
      ...
      <div className="flex flex-1 flex-col px-5 pb-5 pt-5">
        <div className="space-y-4">
          {step === 1 && (
            <>
              <h1 className="text-[1.9rem] font-extrabold leading-tight text-[#1a2748]">
                Lose weight without counting every calorie.
              </h1>
              <p className="text-sm leading-relaxed text-[#5f6e8d]">
          ...
          {step === 2 && (
            <>
              <div>
                <h2 className="text-xl font-bold text-[#1f2a44]">What is your biggest challenge?</h2>
                <p className="mt-1 text-sm text-[#7382a0]">We&apos;ll personalize your daily loop.</p>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        active
                          ? "border-[#5888e8] bg-[#ebf2ff]"
                          : "border-[#d8e1f2] bg-white hover:bg-[#f6f9ff]"
                      )}
                    >
                      <p className="text-sm font-semibold text-[#233055]">{item.label}</p>
                      <p className="mt-0.5 text-xs text-[#7d8cab]">{item.hint}</p>
                    </button>
                  );
                })}
              </div>
          ...
          {step === 3 && (
            <>
              <h2 className="text-xl font-bold text-[#1f2a44]">Set your goal</h2>
              <Input
          ...
          {step === 4 && (
            <>
              <div>
                <h2 className="text-xl font-bold text-[#1f2a44]">Choose 3 daily habits</h2>
                <p className="mt-1 text-sm text-[#7382a0]">Pick micro-habits you can finish quickly.</p>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        selected
                          ? "border-[#39b271] bg-[#ecfff4]"
                          : "border-[#d8e1f2] bg-white"
                      )}
                    >
                      <span className="flex items-center gap-1.5 text-sm font-semibold text-[#233055]">
                        <EmojiIcon emoji={habit.emoji} size={14} />
                        {habit.name}
                      </span>
                      <span className={cn("text-xs font-bold", selected ? "text-[#2f9d64]" : "text-[#97a6c2]")}>{selected ? "Selected" : "Tap"}</span>
                    </button>
                  );
                })}
              </div>

              <p className="text-xs text-[#7d8cab]">{habits.length}/3 selected</p>
            </>
          )}

          {step === 5 && (
            <div className="space-y-4 py-5 text-center">
              <h2 className="text-2xl font-extrabold text-[#1f2a44]">Start a streak today</h2>
              <div className="space-y-2">
                <p className="text-sm font-semibold text-[#2b9e63]">3 days - Momentum</p>
                <p className="text-sm font-semibold text-[#496fd4]">7 days - Habit forming</p>
                <p className="text-sm font-semibold text-[#f1892f]">30 days - Real change</p>
              </div>
            </div>
          )}
          ...
          {step === 6 && (
            <div className="space-y-4 py-2">
              <h2 className="text-2xl font-extrabold text-[#1f2a44]">Welcome!</h2>
              <p className="text-sm text-[#5f6e8d]">Your journey starts today.</p>
              <div className="space-y-2 rounded-2xl border border-[#d8e1f2] bg-[#f6f9ff] p-4">
                <p className="flex items-center gap-1.5 text-sm font-semibold text-[#233055]">
                  <EmojiIcon emoji="🔥" size={14} />
                  Streak: 0
                </p>
                <p className="text-sm font-semibold text-[#233055]">Calories Today: {calorieBudget}</p>
                <p className="text-sm font-semibold text-[#233055]">Habits: {habits.length}</p>
              </div>
            </div>
          )}
        </div>
