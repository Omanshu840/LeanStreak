  ...
  return (
    <div className="space-y-3">
      ...
      <div className={cn(
        ...
        <p className="text-xs font-semibold text-[#6e7a96] uppercase tracking-wide">
          Total Calories
        </p>
        <p className={cn("text-5xl font-extrabold", meta.color)}>
          {result.totalCalories}
        </p>
        <p className="text-sm font-medium text-[#6e7a96]">kcal</p>
        <div className={cn(
      ...
      {/* Steps to burn */}
      <div className="bg-[#f4f7ff] border border-[#d8e1f2] rounded-2xl px-4 py-4">
        <p className="text-xs font-semibold text-[#8a96b0] uppercase tracking-wide mb-3">
          Steps to Burn It Off
        </p>
        <div className="flex items-center gap-3">
          <EmojiIcon emoji="👟" size={24} className="text-[#607195]" />
          <div>
            <p className="text-2xl font-extrabold text-[#1f2a44]">
              {formatSteps(result.steps)}
            </p>
            <p className="text-xs text-[#8a96b0]">at ~70 kcal/1000 steps</p>
          </div>
        </div>
      </div>

      {/* Burn activity breakdown */}
      <div className="bg-[#f4f7ff] border border-[#d8e1f2] rounded-2xl px-4 py-4 space-y-3">
        <p className="text-xs font-semibold text-[#8a96b0] uppercase tracking-wide">
          Or Burn By Activity
        </p>
        {BURN_ACTIVITIES.map(({ key, emoji, label }) => (
          <div key={key} className="flex items-center gap-3">
            <span className="w-8 shrink-0 text-center">
              <EmojiIcon emoji={emoji} size={16} className="mx-auto text-[#607195]" />
            </span>
            <div className="flex-1">
              ...
              {/* Visual effort bar */}
              <div className="h-1.5 bg-[#e4ebfb] rounded-full overflow-hidden">
                <div
                  className={cn(
      ...
      <div className="flex items-center gap-3 bg-purple-50 border border-purple-100
        ...
        <EmojiIcon emoji="🏋️" size={18} className="text-purple-700" />
